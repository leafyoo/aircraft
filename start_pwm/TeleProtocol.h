...
public:
	...
	void Recv(unsigned char* RecvBuf);										
	void SendToTarget(char *IP_addr, int Port, char* SendBuf);		
	void RecvFromTarget(char *IP_addr, int Port, unsigned char* RecvBuf);	
private:
	bool ComType;				
	bool ComRole;
	int SockSrv;			
...
**************************************************************************/
void MySocket::Recv(unsigned char* RecvBuf)
{
	if( (0 == ComType) && (0 == ComRole) )
...
** 注    意：无
**************************************************************************/
void MySocket::RecvFromTarget(char *IP_addr, int Port, unsigned char* RecvBuf)
{
	LinkTarget(IP_addr, Port);
	Recv(RecvBuf);
}
